...
def get_available_cameras():
    ...
    while index < 5: # Check first 5 indices
        cap = cv2.VideoCapture(index, cv2.CAP_DSHOW) # Use CAP_DSHOW for Windows for better performance/compatibility
        if cap.isOpened():
    ...
    def run(self):
        try:
            ...
            process = subprocess.Popen(
                ...
                stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True, creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0
            )
            ...
            if process.returncode == 0:
                ...
                error_msg = f"Installation failed: {stderr or stdout or 'Unknown pip error'}"
...
class InstallSpecificDependencyThread(QThread):
    ...
    def run(self):
        ...
        if not self.pip_package_name:
            ...
            if "tensorflow" in self.dependency_name.lower() or "tflite" in self.dependency_name.lower():
                self.pip_package_name = "tensorflow" # Default to full tensorflow if specific tflite_runtime is not the issue
                self.log_message.emit(f"Specific package for {self.dependency_name} not found, trying generic 'tensorflow'.")
            else:
        ...
        try:
            ...
            if process.returncode == 0:
                self.log_message.emit(f"{self.dependency_name} (as {self.pip_package_name}) installed successfully via pip.")
                # Verification step
                ...
                elif "tflite" in self.dependency_name.lower() or "tensorflow" in self.dependency_name.lower():
                    verification_cmd = ["-c", "import tensorflow as tf; print(tf.__version__)"]


                if verification_cmd:
                    subprocess.run([sys.executable] + verification_cmd, check=True, capture_output=True, text=True, creationflags=subprocess.CREATE_NO_WINDOW if sys.platform == "win32" else 0)
                    self.log_message.emit(f"Successfully verified import of {self.dependency_name}.")
    ...
    def run(self):
        try:
            ...
            if not images_src or not os.path.isdir(images_src):
                self.finished.emit(False, "Could not find 'images' folder. Please ensure it's named 'images' and is within the zip structure.")
                return
            if not labels_src or not os.path.isdir(labels_src):
                self.finished.emit(False, "Could not find 'labels' folder. Please ensure it's named 'labels' and is within the zip structure.")
                return
            if not classes_file_path or not os.path.isfile(classes_file_path):
                self.log_message.emit("classes.txt not found. It must be present in the zip (e.g., next to 'images' and 'labels' folders or in their parent).")
                self.finished.emit(False, "Could not find 'classes.txt'. It must be present in the zip.")
                return

            self.progress.emit("Splitting data...", 50)
            all_images = sorted([f for f in os.listdir(images_src) if f.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp', '.webp'))])
            ...
            with open(self.yaml_path, 'w', encoding='utf-8') as f: # Added encoding
                yaml.dump(data_yaml_content, f, sort_keys=False, default_flow_style=False, allow_unicode=True) # allow_unicode for names
...
class TrainThread(QThread):
    ...
    def run(self):
        try:
            ...
            task_type = "detect" if "seg" not in self.model_name.lower() else "segment" # Check model_name for task
            exact_run_dir = os.path.join(self.project_name, task_type, self.run_name)
            if os.path.exists(exact_run_dir):
                self.progress_update.emit(f"Deleting existing run directory: {exact_run_dir} to ensure a fresh start.")
    ...
    def run(self):
        try:
            ...
            try:
                ...
                if self.source_type != 'image': # Test with a dummy frame for stream sources
                    dummy_h = self.resolution[1] if self.resolution and len(self.resolution) == 2 else 480
                    dummy_w = self.resolution[0] if self.resolution and len(self.resolution) == 2 else 640
                    dummy_frame = np.zeros((dummy_h, dummy_w, 3), dtype=np.uint8)
                    self.model.predict(dummy_frame, device=device_arg, conf=self.confidence_threshold, verbose=False, imgsz=(dummy_h, dummy_w))
                self.log_message.emit(f"Model '{os.path.basename(self.model_path)}' loaded and test prediction successful.")

            except ImportError as e:
                err_str = str(e).lower()
                if "onnxruntime" in err_str:
                    self.log_message.emit(f"ImportError: ONNX Runtime is likely missing for {self.model_path}.")
                    self.finished.emit(f"missing_dependency:onnxruntime:{self.model_path}", self.source_type)
                    return
                # Add more specific checks for other backends if needed
                elif "tensorflow" in err_str or "tflite" in err_str : #Broader check for TF related
                    self.log_message.emit(f"ImportError: TensorFlow/TFLite runtime is likely missing for {self.model_path}.")
                    self.finished.emit(f"missing_dependency:tensorflow:{self.model_path}", self.source_type) # Use generic 'tensorflow'
                    return
                self.log_message.emit(f"Unhandled ImportError during model load: {e}")
                self.finished.emit(f"Error: Missing library for model - {e}", self.source_type)
                return
            except Exception as e: 
                err_str = str(e).lower()
                ...
                if "openvino" in err_str and ("not found" in err_str or "install" in err_str or "inference engine" in err_str):
                     ...
                     self.finished.emit(f"missing_dependency:openvino:{self.model_path}", self.source_type)
                     return
                if ("tensorflow" in err_str or "tflite" in err_str) and ("not found" in err_str or "install" in err_str):
                     self.log_message.emit(f"Error suggests TensorFlow/TFLite is missing for {self.model_path}: {e}")
                     self.finished.emit(f"missing_dependency:tensorflow:{self.model_path}", self.source_type)
                     return
                self.log_message.emit(f"Error during YOLO model initialization or test prediction: {e}")
                self.finished.emit(f"Error: Could not load model or initial predict failed - {e}", self.source_type)
                return
            ...
            if self.source_type == 'webcam':
                try:
                    ...
                    if not self.cap.isOpened():
                        self.finished.emit(f"Error: Could not open webcam {cam_idx}.", self.source_type)
                        return
                    if self.resolution and len(self.resolution) == 2:
                        ...
                        self.log_message.emit(f"Actual webcam resolution set to: {int(actual_width)}x{int(actual_height)}")
                        if not (abs(actual_width - self.resolution[0]) < 10 and abs(actual_height - self.resolution[1]) < 10) : # Allow small tolerance
                            self.log_message.emit(f"Warning: Webcam might not have accepted exact resolution {self.resolution}. Using {actual_width}x{actual_height}.")
                    self.log_message.emit(f"Webcam {cam_idx} opened.")
                except ValueError:
                    self.finished.emit(f"Error: Invalid webcam index '{self.source_path}'.", self.source_type)
                    return
            elif self.source_type == 'video':
                self.cap = cv2.VideoCapture(self.source_path)
                if not self.cap.isOpened():
                    self.finished.emit(f"Error: Could not open video file: {self.source_path}", self.source_type)
                    return
                self.log_message.emit(f"Video file opened: {self.source_path}")
            elif self.source_type == 'image':
                img = cv2.imread(self.source_path)
                if img is None:
                    self.finished.emit(f"Error: Could not open image file: {self.source_path}", self.source_type)
                    return
                self.log_message.emit(f"Image file opened: {self.source_path}. Running prediction...")
                
                results = self.model.predict(img, device=device_arg, conf=self.confidence_threshold, verbose=False)
                ...
                self.finished.emit("Error: Invalid source type for inference.", self.source_type)
                return
            ...
            while self._is_running and self.cap.isOpened():
                ...
                results = self.model.predict(frame, device=device_arg, conf=self.confidence_threshold, verbose=False)
            ...
            if "onnxruntime" in err_str:
                ...
                self.finished.emit(f"missing_dependency:onnxruntime:{self.model_path}", self.source_type)
                 ...
                 self.log_message.emit(f"Delayed ImportError: TensorFlow/TFLite runtime is likely missing for {self.model_path}.")
                 self.finished.emit(f"missing_dependency:tensorflow:{self.model_path}", self.source_type)
            else:
                self.log_message.emit(f"Unhandled Delayed ImportError during inference: {e}")
                self.finished.emit(f"Error: Missing library during inference - {e}", self.source_type)
        except Exception as e:
            ...
            if "onnxruntime" in err_str_runtime and ("not found" in err_str_runtime or "install" in err_str_runtime):
                ...
                self.finished.emit(f"missing_dependency:onnxruntime:{self.model_path}", self.source_type)
                 ...
                 self.finished.emit(f"missing_dependency:openvino:{self.model_path}", self.source_type)
            elif ("tensorflow" in err_str_runtime or "tflite" in err_str_runtime) and ("not found" in err_str_runtime or "install" in err_str_runtime):
                 self.log_message.emit(f"Runtime Error suggests TensorFlow/TFLite is missing: {e}")
                 self.finished.emit(f"missing_dependency:tensorflow:{self.model_path}", self.source_type)
            else:
...
class ModernYoloGUI(QMainWindow):
    ...
    def __init__(self):
        ...
        toggle_layout.addWidget(QLabel("Hide"))
        ...
        self.toggle_console_button.setFixedSize(QSize(44, 22))
        toggle_layout.addWidget(self.toggle_console_button)
        toggle_layout.addWidget(QLabel("Show"))
    ...
    def update_all_icons(self):
        ...
        if hasattr(self, 'load_dataset_button'): self.load_dataset_button.setIcon(self.get_icon("dataset_load"))
        if hasattr(self, 'train_button'): self.train_button.setIcon(self.get_icon("train_action"))
        if hasattr(self, 'load_trained_model_button'): self.load_trained_model_button.setIcon(self.get_icon("model_load"))
        
        if hasattr(self, 'start_webcam_button'): self.update_start_webcam_button_state() 
        if hasattr(self, 'load_media_button'): self.load_media_button.setIcon(self.get_icon("media_load"))
        if hasattr(self, 'stop_inference_button'): self.stop_inference_button.setIcon(self.get_icon("stop_inference"))
        if hasattr(self, 'export_model_button'): self.export_model_button.setIcon(self.get_icon("model_export"))
        if hasattr(self, 'install_ultralytics_button'): self.install_ultralytics_button.setIcon(self.get_icon("yolo_icon"))
    ...
    def create_home_page(self):
        ...
        # Create a container widget for the background image
        ...
        content_layout.setContentsMargins(20, 20, 20, 20)  # Increased padding
        content_layout.setSpacing(15)
        ...
        <p style='font-size: 11pt;'>Ensure Ultralytics is installed via the Settings page for full functionality.</p>
        """
        ...
        content_layout.addWidget(self.intro_text_label)
    ...
    def create_data_page(self):
        ...
        self.load_dataset_button = QPushButton(self.get_icon("dataset_load"), " Load Custom Dataset (.zip)")
    ...
    def create_train_page(self):
        ...
        self.train_button = QPushButton(self.get_icon("train_action"), " Start Training") 
        ...
        self.load_model_after_train_checkbox = QCheckBox("Load this model in Deploy tab after training")
    ...
    def create_deploy_page(self):
        ...
        self.load_trained_model_button = QPushButton(self.get_icon("model_load"), " Load Model for Inference") 
        ...
        self.load_media_button = QPushButton(self.get_icon("media_load"), " Load Video/Image File") 
        ...
        self.stop_inference_button = QPushButton(self.get_icon("stop_inference"), " Stop File Inference") 
        self.stop_inference_button.setToolTip("Stops inference from video/image file if running.")
        ...
        self.export_model_button = QPushButton(self.get_icon("model_export"), " Convert & Export Model") 
    ...
    def create_settings_page(self):
        ...
        self.install_ultralytics_button = QPushButton(self.get_icon("yolo_icon"), " Install/Verify Ultralytics") 
    ...
    def create_about_page(self):
        ...
        # Basic styling for this page can be part of the main QSS or specific here
        # page.setStyleSheet("QLabel { font-size: 10pt; }") 
        ...
        # Use a less prominent color for version, can be styled in QSS
        version_label.setStyleSheet("font-size: 9pt; color: gray;") # Example inline
        layout.addWidget(version_label)

        layout.addSpacing(20)
        
        heart_color = self.current_palette_colors.get("label_header_color", "red") # Use a theme color or fallback
        made_with_html = f"""
        <p style='text-align: center; font-size: 12pt; color: {self.current_palette_colors.get("text_color", "#000000")};'>
            Made with <span style='color: {heart_color}; font-size: 16pt;'>&hearts;</span> by Midhun Mathew
        </p>
        """
        ...
        description_label.setStyleSheet(f"padding: 0 20px; color: {self.current_palette_colors.get('text_color', '#000000')};") 
        layout.addWidget(description_label)
        
        layout.addSpacing(20)

        link_color_for_html = self.current_palette_colors.get("label_header_color", "#007AFF")
        support_link_html = f"""
        <p style='text-align: center;'>
            <a href='https://www.buymeacoffee.com/midhunmathew' style='color: {link_color_for_html}; text-decoration: none;'>
                Support the Developer (Buy Me a Coffee)
            </a><br><br>
            <a href='https://www.linkedin.com/in/midhun-mathew-674997213/' style='color: {link_color_for_html}; text-decoration: none;'>
                Connect on LinkedIn
            </a>
        </p>
        """
    ...
    def apply_stylesheet(self, theme="light"):
        ...
        elif sys.platform == "darwin": font.setFamily("SF Pro Display") 
        else: font.setFamily("Cantarell") 
        QApplication.setFont(font)
        
        p = self.light_palette if self.current_theme == "light" else self.dracula_palette
        self.current_palette_colors = p

        # Add background image styling with absolute path
        background_image_path = os.path.abspath(os.path.join("icons", "image.jpg"))
        if os.path.exists(background_image_path):
            # Convert Windows path to URL format for Qt
            background_image_url = background_image_path.replace('\\', '/')
            background_style = f"""
                ...
                QWidget#contentOverlay {{
                    background-color: {p["window_bg"]}aa;  /* Semi-transparent background */
                    border-radius: 10px;
                    margin: 20px;  /* Add some margin around the content */
                }}
                QGroupBox#statsGroup {{
                    background-color: {p["group_bg"]}dd;  /* More opaque background for better readability */
                }}
            """
            self.log(f"Background image loaded from: {background_image_url}")
        else:
            background_style = ""
            self.log(f"Background image not found at: {background_image_path}")
        ...
        qss = f"""
            ...
            QGroupBox {{
                ...
                font-weight: bold; color: {p["text_color"]}; /* GroupBox text color */
            }}
            QGroupBox::title {{
            ...
            QLabel#pageTitle {{ 
                font-size: 18pt; font-weight: bold; color: {p["label_header_color"]}; 
                padding-bottom: 10px; margin-top: 5px;
            }}
            QLabel#introText {{ font-size: 10pt; line-height: 1.5; color: {p["text_color"]}; }}
            ...
            QPushButton:disabled {{ background-color: {p["disabled_bg"]}; color: {p["disabled_text"]}; border-color: {p["disabled_text"]};}}
            ...
            QPushButton#toggleConsoleButton::before {{
                content: '';
                position: absolute;
                width: 18px;
                height: 18px;
            ...
            QPushButton#toggleConsoleButton:checked {{
                background-color: {p["label_header_color"]};
            }}

            QPushButton#toggleConsoleButton:checked::before {{
                left: 24px;
            }}
            ...
            QPushButton#toggleConsoleButton:checked:hover {{
                background-color: {p["button_hover_bg"]};
            }}
    ...
    def change_theme(self, theme_name_full):
        ...
        if self.current_theme != new_theme_base:
            ...
            self.apply_stylesheet(self.current_theme) # apply_stylesheet will now use the updated self.current_theme
        ...
        if cams:
            ...
            if current_selection in [f"Webcam {i}" for i in cams]:
                self.webcam_combo.setCurrentText(current_selection)
                 ...
                 self.webcam_combo.setCurrentIndex(0) # Default to first if previous not found
    ...
    def update_start_webcam_button_state(self):
        if hasattr(self, 'start_webcam_button'): # Ensure button exists
            if self.inference_thread and self.inference_thread.isRunning() and self.inference_thread.source_type == 'webcam':
    ...
    def update_inference_controls_state(self):
        ...
        is_any_inference_running = self.inference_thread is not None and self.inference_thread.isRunning()
        is_webcam_inference_running = is_any_inference_running and self.inference_thread.source_type == 'webcam'
        is_file_inference_running = is_any_inference_running and self.inference_thread.source_type in ['video', 'image']

        if hasattr(self, 'start_webcam_button'):
            self.start_webcam_button.setEnabled(can_perform_actions_with_model and webcam_available and not is_file_inference_running)
            self.update_start_webcam_button_state() 

        if hasattr(self, 'webcam_resolution_combo'):
             self.webcam_resolution_combo.setEnabled(can_perform_actions_with_model and webcam_available and not is_any_inference_running)
        if hasattr(self, 'load_media_button'): self.load_media_button.setEnabled(can_perform_actions_with_model and not is_any_inference_running)
        if hasattr(self, 'stop_inference_button'): self.stop_inference_button.setEnabled(is_file_inference_running) 
        
        if hasattr(self, 'export_model_button'): self.export_model_button.setEnabled(can_perform_actions_with_model and not is_any_inference_running)
        if hasattr(self, 'load_trained_model_button'): self.load_trained_model_button.setEnabled(not is_any_inference_running) 

        if not model_loaded:
            if hasattr(self, 'inference_model_status_label'): self.inference_model_status_label.setText("No model loaded. Please load a model file.")
            if hasattr(self, 'export_status_label'): self.export_status_label.setText("Export Status: Load a model first.")
        elif not ultralytics_ready:
             if hasattr(self, 'inference_model_status_label'): self.inference_model_status_label.setText("Ultralytics not installed. Please install from Settings.")
             if hasattr(self, 'export_status_label'): self.export_status_label.setText("Export Status: Ultralytics not installed.")
        else: 
            if not is_any_inference_running: 
                if hasattr(self, 'inference_model_status_label'): self.inference_model_status_label.setText(f"Active Model: {os.path.basename(self.trained_model_path_for_inference if self.trained_model_path_for_inference else 'N/A')}")
                if hasattr(self, 'export_status_label'): self.export_status_label.setText("Export Status: Ready")
    ...
    def install_specific_dependency(self, dep_name, model_path_context):
        ...
        if hasattr(self, 'start_webcam_button'): self.start_webcam_button.setEnabled(False)
        if hasattr(self, 'load_media_button'): self.load_media_button.setEnabled(False)
    ...
    def select_dataset_zip(self):
        ...
        if fileName:
            ...
            if hasattr(self, 'train_button'): self.train_button.setEnabled(False) 
    ...
    def start_data_preparation(self):
        ...
        data_processing_subdir = os.path.join(self.work_dir, "dataset_processing", os.path.splitext(os.path.basename(self.dataset_zip_path))[0])
        try:
            if os.path.exists(data_processing_subdir): shutil.rmtree(data_processing_subdir) # Clean before use
    ...
    def on_data_prep_complete(self, success, path_or_message):
        if success:
            ...
            self.data_yaml_path_label.setText(f"Data YAML: {os.path.basename(self.data_yaml_path)}") 
    ...
    def start_training_process(self):
        if not self.data_yaml_path:
            ...
            QMessageBox.warning(self, "Dataset Missing", "Please prepare a dataset on the 'Data' tab first.")
            return
        if not ULTRALYTICS_INSTALLED:
            ...
            QMessageBox.warning(self, "Ultralytics Missing", "Ultralytics is not installed. Please install it from the 'Settings' tab.")
            return
    ...
    def on_training_session_finished(self, success, model_path_or_message):
        if hasattr(self, 'train_button'): self.train_button.setEnabled(ULTRALYTICS_INSTALLED and self.data_yaml_path is not None) 
        if success:
            ...
            self.trained_model_path_display_label.setText(f"Latest Trained Model: {os.path.basename(model_path_or_message)}")
            
            if self.load_model_after_train_checkbox.isChecked():
                self.log(f"Automatically loading trained model '{os.path.basename(model_path_or_message)}' into Deploy tab.")
    ...
    def toggle_webcam_inference_action(self):
        if self.inference_thread and self.inference_thread.isRunning() and self.inference_thread.source_type == 'webcam':
            ...
            if self.inference_thread and self.inference_thread.isRunning():
                QMessageBox.warning(self, "Inference Busy", "Another inference (e.g., from a file) is currently running. Please stop it first.")
                return
    ...
    def run_live_inference(self, source_type, source_path):
        if not self.trained_model_path_for_inference:
            ...
            QMessageBox.warning(self, "Model Missing", "No model is loaded. Please load a model on the 'Deploy' tab.")
            return
        if not ULTRALYTICS_INSTALLED:
        ...
        if self.inference_thread and self.inference_thread.isRunning():
            ...
            QMessageBox.information(self, "Busy", "An inference process is already running. Please stop it first.")
            return
        ...
        webcam_res = self.webcam_resolutions.get(selected_resolution_text) if source_type == 'webcam' else None
    ...
    def run_webcam_inference(self):
        ...
        try:
            ...
            QMessageBox.warning(self, "Webcam Error", f"Invalid webcam selection: {cam_text}")
    ...
    def stop_live_inference(self):
        if self.inference_thread and self.inference_thread.isRunning():
            self.log(f"Sending stop signal to {self.inference_thread.source_type} inference process...")
    ...
    def update_inference_video_feed(self, q_image, fps, object_count):
        ...
        scaled_image = q_image.scaled(640, 480, Qt.KeepAspectRatio, Qt.SmoothTransformation)
    ...
    def on_live_inference_finished(self, message, source_type): 
        ...
        if message.startswith("missing_dependency:"):
            ...
            if len(parts) == 3:
                ...
                if reply == QMessageBox.Yes:
                 ...
                 self.video_feed_label.setText(f"Output Preview (Error parsing dependency message)")
            self.inference_thread = None 
            self.update_inference_controls_state()
            return
        ...
        if message != "Image inference complete.": # Don't clear stats for a successfully displayed image
            self.fps_label.setText("FPS: --")
            self.detected_objects_label.setText("Objects: --")
    ...
    def run_export_model(self):
        if not self.trained_model_path_for_inference:
            ...
            QMessageBox.warning(self, "Model Missing", "No model loaded. Please load a model before exporting.")
            return
        if not ULTRALYTICS_INSTALLED:
            ...
            QMessageBox.warning(self, "Ultralytics Missing", "Ultralytics is not installed. Please install it from Settings.")
            return
        ...
        if hasattr(self, 'export_model_button'): self.export_model_button.setEnabled(False) 
    ...
    def on_export_model_finished(self, success, path_or_message):
        ...
        if hasattr(self, 'export_model_button'): self.export_model_button.setEnabled(True) 


    def closeEvent(self, event):
        self.log("Application closing. Stopping active threads...")
        threads_to_stop = [
            'inference_thread', 'dependency_install_thread', 
            'train_thread', 'data_prep_thread', 'install_thread', 'export_thread'
        ]
        for thread_attr_name in threads_to_stop:
            ...
            if thread_instance and thread_instance.isRunning():
                ...
                if hasattr(thread_instance, 'stop') and callable(getattr(thread_instance, 'stop')):
                ...
                if not thread_instance.wait(2000): # Wait up to 2 seconds
                    self.log(f"{thread_attr_name} did not stop gracefully. Terminating...")
...
def create_dummy_icons_if_needed():
    ...
    themes = ["light", "dark"]
    dummy_icon_list = [
        "home", "data", "train", "deploy", "settings", "about", "app_icon",
        "console_show", "console_hide", "dropdown_arrow", # Added dropdown_arrow
        "dataset_load", "train_action", "model_load", "media_load", 
        "webcam_start", "webcam_stop", "stop_inference", "model_export", "yolo_icon"
    ]
        ...
        for icon_name_base in dummy_icon_list:
            ...
            if not os.path.exists(icon_file):
                try:
                    ...
                    img.fill(Qt.transparent if icon_name_base != "app_icon" else (Qt.blue if theme == "light" else Qt.darkGray) ) 
    ...
    if not os.path.exists(generic_app_icon):
        try:
            ...
            if not img.save(generic_app_icon): print(f"Failed to save dummy generic app_icon.png")
        except Exception as e: print(f"Error creating generic app_icon: {e}")
...
if __name__ == '__main__':
    ...
    if not os.path.exists(dummy_icon_dir):
        ...
        dummy_icons = [
            "home.png", "data.png", "train.png", "deploy.png", "settings.png", "about.png",
            "console_show.png", "console_hide.png", "dropdown_arrow.png",
            "dataset_load.png", "train_action.png", "model_load.png", "media_load.png", 
            "webcam_start.png", "webcam_stop.png", "stop_inference.png", "model_export.png", "yolo_icon.png"
        ]
